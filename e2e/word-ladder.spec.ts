...
test.describe('Word Ladder route', () => {
  test('protects the draft and returns one hierarchy level at a time', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Choose Happy as the answer' })).toBeVisible()
    await expect(page.getByRole('button', { name: 'Choose Playful as the answer' })).toBeVisible()

    await page.getByRole('button', { name: 'Back one level' }).click()
    await expect(page.getByRole('button', { name: 'Choose Happy as the answer' })).toBeVisible()
    await expect(page.getByRole('button', { name: 'Choose Playful as the answer' })).toHaveCount(0)
    await expect(page.getByRole('button', { name: 'Explore more specific words under Playful' })).toBeVisible()

    await page.getByRole('button', { name: 'Back one level' }).click()
    await expect(page.getByRole('button', { name: 'Explore more specific words under Happy' })).toBeVisible()
  ...
  test('selects a broad path level and keeps Reflection interruption-free', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Explore more specific words under Happy' }).click()
    await page.getByRole('button', { name: 'Explore more specific words under Playful' }).click()
    await page.getByRole('button', { name: 'Choose Happy as the answer' }).click()
  ...
  test('compares a selected intermediary only within its complete sibling group', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Explore more specific words under Happy' }).click()
    await page.getByRole('button', { name: 'Explore more specific words under Playful' }).click()
    await page.getByRole('button', { name: 'Choose Playful as the answer' }).click()
  ...
  test('does not offer comparison for a leaf group without complete reviewed descriptions', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Explore more specific words under Happy' }).click()
    await page.getByRole('button', { name: 'Explore more specific words under Playful' }).click()
    await page.getByRole('button', { name: 'Select Energized' }).click()
  ...
  test('localizes hierarchy controls and stays within the mobile viewport', async ({ page }) => {
    ...
    await openArrival(page)
    await page.getByTestId('arrival-words').click()
    await page.getByRole('button', { name: 'Explorează cuvinte mai precise sub Fericit' }).click()

    await expect(page.getByRole('button', { name: 'Alege Fericit ca răspuns' })).toBeVisible()
    await expect(page.getByRole('button', { name: 'Înapoi cu un nivel' })).toBeVisible()
    await expect(page.getByRole('button', { name: 'Explorează cuvinte mai precise sub Jucăuș' })).toBeVisible()
  ...
  test('keeps stopping prominent and comparison reversible on a narrow screen', async ({ page }) => {
    ...
    await page.getByTestId('arrival-words').click()
    await page.getByRole('button', { name: 'Explorează cuvinte mai precise sub Fericit' }).click()
    await page.getByRole('button', { name: 'Explorează cuvinte mai precise sub Jucăuș' }).click()
    ...
    await page.getByRole('button', { name: 'Alege Jucăuș ca răspuns' }).click()

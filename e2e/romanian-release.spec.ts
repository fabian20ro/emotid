...
test.describe('Romanian release journeys', () => {
  test('Quick reaches a localized reflection and close state', async ({ page }) => {
    ...
    await expect(page.getByText(/Ai ales:.*Poți păstra sau schimba alegerea/i)).toBeVisible()
    ...
    await expect(page.locator('.more-context')).toHaveCount(0)
    await expect(page.locator('.meaning-block')).toBeVisible()
    await expect(page.locator('.meaning-block')).not.toContainText(/experimentezi un semnal|sistemul tău răspunde/i)

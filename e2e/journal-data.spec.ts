...
test.describe('Journal data trust', () => {
  ...
  test('localizes stored body region and sensation details in Romanian', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Încearcă un pas mic' }).click()
    await page.getByRole('button', { name: 'Oprește-te puțin și observă ce pare realizabil în continuare.' }).click()
    await page.getByRole('button', { name: 'Păstrează acest pas' }).click()
    await expect(page.getByTestId('today-screen')).toBeVisible()

    await page.getByRole('button', { name: 'Jurnal', exact: true }).click()
    await expect(page.getByRole('heading', { name: 'Primele tale reflecții' })).toBeVisible()
